...
async def run_coordinator_async(runner: InMemoryRunner, request: str):
    """Runs the coordinator agent with a given request and delegates (async version)."""
...
def run_coordinator(runner: InMemoryRunner, request: str):
    """Synchronous wrapper for the async coordinator function."""
    return asyncio.run(run_coordinator_async(runner, request))
